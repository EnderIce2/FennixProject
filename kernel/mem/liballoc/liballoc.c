...
unsigned int l_inuse = 0;	  //< The amount of memory in use (liballoc_malloc'ed).
#endif
...
static void dump_array()
{
	...
	printf("Memory in used (liballoc_malloc'ed): %i\n", l_inuse);
...
static inline struct boundary_tag *absorb_right(struct boundary_tag *tag)
{
	...
	remove_tag(right); // Remove right from liballoc_free pages.
...
static struct boundary_tag *allocate_new_tag(unsigned int size)
{
	...
	tag = (struct boundary_tag *)RequestPage(pages);
...
void *liballoc_malloc(size_t size)
{
	...
	// We have a liballoc_free page.  Remove it from the liballoc_free pages list.
