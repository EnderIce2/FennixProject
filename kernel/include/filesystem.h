...
#define MkdirFSFunction(name) void name(FileSystem::FileSystemNode *Node, char *Name, uint16_t NameLength)
    ...
    {
    public:
        FileSystemNode *AddFileSystem(struct FileSystemOpeations *Node, uint64_t Mode, string Name, int Flags);
        Device();
        ~Device();
    };
    ...
    {
    public:
        FileSystemNode *MountFileSystem(FileSystemOpeations *Node, uint64_t Mode, string Name);
