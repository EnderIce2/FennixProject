...
#endif
	// debug("------ Memory data ---------------");
	// debug("System memory allocated: %i bytes", l_allocated);
	// debug("Memory in used (malloc'ed): %i bytes", l_inuse);
	// debug("Warning count: %i", l_warningCount);
	// debug("Error count: %i", l_errorCount);
	// debug("Possible overruns: %i", l_possibleOverruns);
#ifdef DEBUG
	while (maj != NULL)
	{
		// debug("%llx: total = %i, used = %i", maj, maj->size, maj->usage);
		min = maj->first;
		while (min != NULL)
		{
			// debug("   %llx: %i bytes", min, min->size);
			min = min->next;
		}
		maj = maj->next;
	}
...
static struct liballoc_major *allocate_new_page(unsigned int size)
{
	...
	maj = (struct liballoc_major *)syscall_requestPages(st);
	if (maj == NULL)
	{
		l_warningCount += 1;
#if defined DEBUG || defined INFO
		// warn("RequestPages( %i ) -> return NULL", st);
#endif
		// err("Out of memory.");
...
#ifdef DEBUG
	// printf("Resource allocated %llx of %i pages (%i bytes) for %i size.", maj, st, maj->size, size);
	// printf("Total memory usage = %i KB", (int)((l_allocated / (1024))));
#endif
	return maj;
}

void *libPREFIX(malloc)(size_t req_size)
{
	int startedBet = 0;
...
#if defined DEBUG || defined INFO
#ifdef DEBUG
		// debug("Initialization of liballoc %s", VERSION);
#endif
		liballoc_dump();
#endif
...
#ifdef DEBUG
		// trace("Set up first memory major %llx", l_memRoot);
#endif
	}
#ifdef DEBUG
	// trace("%llx -> malloc( %i ): ", __builtin_return_address(0), size);
#endif
	...
	while (maj != NULL)
	{
		...
		{
#ifdef DEBUG
			// printf("CASE 1: Insufficient space in block %llx", maj);
#endif
...
#endif
#if defined DEBUG || defined INFO
	// warn("malloc( %i ) returning NULL.", size);
	liballoc_dump();
#endif
	return NULL;
}

void libPREFIX(free)(void *ptr)
{
	struct liballoc_minor *min;
	struct liballoc_major *maj;
	...
	if (min->magic != LIBALLOC_MAGIC)
	{
...
#if defined DEBUG || defined INFO
			// err("Possible 1-3 byte overrun for magic %llx != %llx", min->magic, LIBALLOC_MAGIC);
#endif
		}
		...
		{
#if defined DEBUG || defined INFO
			// err("Multiple free( %llx ) attempt from %llx.", ptr, __builtin_return_address(0));
#endif
		}
		else
		{
#if defined DEBUG || defined INFO
			// err("Bad free( %llx ) called from %llx", ptr, __builtin_return_address(0));
#endif
		}
...
#ifdef DEBUG
	// printf("%llx libPREFIX(free)( %llx ): ",  __builtin_return_address(0), ptr);
#endif
	...
	if (maj->first == NULL) // Block completely unused.
	{
		...
		l_allocated -= maj->size;
		syscall_freePages(maj, maj->pages);
	}
	else
	{
		if (l_bestBet != NULL)
...
void *libPREFIX(calloc)(size_t nobj, size_t size)
{
	...
	void *p;
	real_size = nobj * size;
	p = libPREFIX(malloc)(real_size);
	liballoc_memset(p, 0, real_size);
	return p;
}

void *libPREFIX(realloc)(void *p, size_t size)
{
	void *ptr;
	struct liballoc_minor *min;
